    ...
    <div class="page-container">
        ...
        <div class="page-title-head d-flex align-items-sm-center flex-sm-row flex-column gap-2">
            <div class="flex-grow-1">
                <h4 class="fs-18 fw-semibold mb-0">Editar Reserva de Sala</h4>
            </div>
        ...
        <div class="col-xl-7 col-lg-12">
            <div class="card">
                ...
                <form action="{{route('cliente-EditarPreReservaSala')}}" method="POST">
                    @csrf
                    ...
                    <div id="horarios-container" style="display: none;">
                        <div class="mt-3 mb-2">
                            ...
                            <select id="hora_inicio" name="hora_inicio" class="form-control" required>
                                ...
                                <option value="{{$preReserva->hora_inicio}}">{{$preReserva->hora_inicio}}</option>
                            </select>
                        </div>
                        ...
                        <div class="mt-3 mb-2">
                            ...
                            <select id="hora_fim" name="hora_fim" class="form-control" required>
                                <option value="{{$preReserva->hora_fim}}">{{$preReserva->hora_fim}}</option>
                            </select>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        function buscarHorarios() {
            ...
            var data_original = "<?= $preReserva->data ?>";
            if (dataSelecionada) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.horarios_inicio.length > 0) {
                            ...
                            if(dataSelecionada != data_original){
                            $("#hora_inicio").empty();
                            $("#hora_fim").empty();
                            }
        ...
        // Executa a função ao alterar a data
        $("#data").on("change", buscarHorarios);
    });
</script>
                    

                    <button type="submit" class="btn btn-primary mt-3">Editar Reserva</button>
                </form> 
